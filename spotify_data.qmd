...
p3 <- songs %>%
  select(c('genre', feature_names)) %>%
  # mutate(speechiness = log(acousticness)) %>% 
  pivot_longer(cols = feature_names) %>%
  ggplot(aes(x = value)) +
  geom_density(aes(color = genre), alpha = 0.9) +
  facet_wrap(~name, ncol = 3, scales = 'free') +
...
songs.short <- songs[predictors]

split1 <- initial_split(songs.short, prop = .7) # samples index 
...
metal.short <- metal.nonmetal[c("metal", "key", "mode", "danceability",
                                "energy", "loudness", "speechiness", 
                                "acousticness", "instrumentalness", 
                                "valence", "tempo", "duration", "liveness")]
